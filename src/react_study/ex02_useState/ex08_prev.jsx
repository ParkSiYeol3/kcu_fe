    ...
    <div style={{ 
      ...
      <form onSubmit={handleSubmit} style={cardStyle}>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <label style={{ fontSize: "14px", color: "#FDFDFD" }}>캐릭터 이름</label>
          <input type="text" name="name" value={form.name} onChange={changeHandler} style={fieldStyle} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <label style={{ fontSize: "14px", color: "#FDFDFD" }}>캐릭터 설정</label>
      ...
      {card.map((item, idx) => (
        <div key={idx} style={cardStyle}>
          ...
          <div style={{ color: "#FDFDFD" }}>{item.desc}</div>
          <button
